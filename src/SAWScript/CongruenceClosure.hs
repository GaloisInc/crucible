...
-- |
splitClass :: (OrdFoldable f, Traversable f)
           => [Term f] -> Set (CCSet f) -> Set (CCSet f)
...
-- configurations.
mayAliases :: (OrdFoldable f, Traversable f)
           => [[Term f]] -> CCSet f -> Set (CCSet f)
mayAliases l s = List.foldr splitClass (Set.singleton s) l
