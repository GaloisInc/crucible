...
module Lang.Crucible.Solver.SimpleBackend.Heman
( SimulationResult(..)
, Option(..)
, generateCircuit
) where
...
-- TODO: Inputs will need to be collected together
writeInput :: Synthesis t -> Word64 -> Word64 -> IO (NameType BaseIntegerType)
writeInput synth out inpId = do
  Sys.hPrintf (synthesisOut synth) "input wire%d index%d : ct\n" out inpId
  return (Ref out)
...
writeOutput :: Synthesis t -> [Elt t tp] -> IO ()
writeOutput synth es = mapM_ (eval synth) es
  ...
  case nonceEltApp ae of
    FnApp fn args -> case T.unpack (solverSymbolAsText (symFnName fn)) of
        ...
        BaseIntegerRepr -> do
          ...
          case (Ctx.intIndex 0 sz, Ctx.intIndex 1 sz) of
            (Just (Some zero), Just (Some one)) -> do
              ...
              -- TODO: remove this
              Ref <$> writeMul synth (indexValue (nonceEltId ae)) base by
      ...
      "julia_dotProd!" -> case symFnReturnType fn of
        ...
        _ -> fail "The improbable happened: dot product should only return Integer type"
      x -> fail $ "1Not supported: " ++ show x
    _ -> fail $ "Not supported"

-- Circuit generation functions

-- TODO: this should distinguish between inputs and intermediates
mkVarName :: Word64 -> String
mkVarName var = "v" ++ show var

writeNaryOp :: String -> Synthesis t -> Word64 -> [Word64] -> IO Word64
writeNaryOp op synth out args = do
  let lhs = mkVarName out
      rhs = intercalate op (map mkVarName args)
  Sys.hPutStrLn (synthesisOut synth) (lhs ++ " = " ++ rhs)
  return out

writeMul :: Synthesis t -> Word64 -> Word64 -> Word64 -> IO Word64
writeMul synth out in1 in2 = writeNaryOp " * " synth out [in1, in2]

writeMulN :: Synthesis t -> Word64 -> [Word64] -> IO Word64
writeMulN = writeNaryOp " * "

writeSumN :: Synthesis t -> Word64 -> [Word64] -> IO Word64
writeSumN = writeNaryOp " + "

writeNeg :: Synthesis t -> Word64 -> Word64 -> IO Word64
writeNeg synth out ref = do
  Sys.hPrintf (synthesisOut synth) "%s = - %s\n" (mkVarName out) (mkVarName ref)
  return out

-- TODO: this is wrong for now, and we need an encoder to fix it.
...
-- TODO: this needs to collect the outputs and put them at the top of the
-- file, but that's it.
...
writeDotProd synth out args = do
  let lhs = mkVarName out
      ...
      (xs, ys) = splitList args
      prods = zipWith (\x y -> mkVarName x ++ " * " ++ mkVarName y) xs ys
      rhs = intercalate " + " prods
  Sys.hPutStrLn (synthesisOut synth) (lhs ++ " = " ++ rhs)
  return out
