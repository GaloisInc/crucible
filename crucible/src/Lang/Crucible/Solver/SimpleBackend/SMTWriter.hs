...
import           Lang.Crucible.Solver.Interface (ArrayResultWrapper(..), IndexLit(..))
...
getBaseSMT_Type :: SimpleBoundVar t tp -> SMTCollector t h (TypeMap tp)
...
checkVarTypeSupport :: SimpleBoundVar n tp -> SMTCollector n h ()
...
predSMTExpr e0 = do
  ...
  h <- asks scConn
  liftIO $ updateProgramLoc h (nonceEltLoc e0)
  case nonceEltApp e0 of
    ...
    MapOverArrays f idx_types arrays -> do
      ...
      let evalArray :: forall idx itp etp
                     . ArrayResultWrapper (Elt t) (idx Ctx.::> itp) etp
...
checkIntegerSupport :: Elt t tp -> SMTCollector t h ()
...
checkLinearSupport :: Elt t tp -> SMTCollector t h ()
...
checkNonlinearSupport :: Elt t tp -> SMTCollector t h ()
...
checkComputableSupport :: Elt t tp -> SMTCollector t h ()
...
defineFn :: SMTWriter h
         => WriterConn t h
         -> Text
         -> Ctx.Assignment (SimpleBoundVar t) a
         -> Elt t r
...
mkExpr (NonceAppElt ea) =
  cacheWriterResult (nonceEltId ea) DeleteOnPop $
    predSMTExpr ea
mkExpr (AppElt ea) =
  cacheWriterResult (appEltId ea) DeleteOnPop $ do
    appSMTExpr ea
mkExpr (BoundVarElt var) = do
