...
import           Lang.Crucible.Simulator.GlobalState as GS
...
instance HasSimState (MSS_State sym) where
  ...
  globalMuxFn s =
      isSolverProof ctx $
        GS.globalMuxFn (ctx^.ctxSymInterface) (ctxIntrinsicMuxFns ctx)
    where ctx = s^.stateContext

  globalPushBranch s =
      isSolverProof ctx $
        GS.globalPushBranch (ctx^.ctxSymInterface) (ctxIntrinsicMuxFns ctx)
    where ctx = s^.stateContext

  globalAbortBranch s =
      isSolverProof ctx $
        GS.globalAbortBranch (ctx^.ctxSymInterface) (ctxIntrinsicMuxFns ctx)
    where ctx = s^.stateContext
