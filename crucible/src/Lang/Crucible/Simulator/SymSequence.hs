...
reverseAsPrefixResult sym =
  \case
    ...
    Ite p l r -> do
      l' <- reverseAsPrefixResult sym l
      r' <- reverseAsPrefixResult sym r
      pure (Ite p l' r')
  ...
  \case
    ...
    Ite p l' r' ->
      case asConstantPred p of
        Just True -> andAsPrefixResult sym l l'
        Just False -> andAsPrefixResult sym l r'
        Nothing -> do
...
eqSymSequencePrefix sym eq mux l r =
  case (l, r) of
    ...
    (_, SymSequenceMerge _ p t f) ->
      reverseAsPrefixResult sym =<< eqIte p t f l
    ...
    (SymSequenceCons _ x xs, SymSequenceAppend {}) -> do
      ...
      case mS of
        Unassigned -> pure (LeftLonger (truePred sym) l)
        PE ok (y, ys) -> do
          heads <- eq x y
  ...
  where
    ...
    appendPfx us vs xs ys =
      \case
        ...
        Ite p t f ->
          case asConstantPred p of
            Just True -> appendPfx us vs xs ys t
            Just False -> appendPfx us vs xs ys f
            Nothing -> do
