...
-- This option allows the module to compile even with warnings
-- generated by the GHC pattern completeness checker, which cannot
-- reasonably complete checking the pattern matches produced by the
-- Template Haskell generated equality and ordering tests on App.
{-# OPTIONS_GHC -Wwarn #-}

-- This option, if enabled, currently allows the GHC (8.0.2) coverage
-- checker to check the large pattern matches produced below.  It is
-- disabled for now as it causes GHC to consume a large amount of
-- memory, to the point that it crashes on some of the build servers.
