...
module Lang.Crucible.SymIO where
...
import           Lang.Crucible.SymIO.Extension.Syntax
...
instance IsExprBuilder sym => IntrinsicClass sym "VFS_files" where
...
evalStmt sym = eval
  where
    ...
    eval (VFS_Read fvar (regValue -> fhandle) size) = do
      ...
      off <- liftIO $ bvLit sym PtrWidth (BV.mkBV PtrWidth (bytesToInteger size))
    ...
    eval (VFS_Write fvar (regValue -> fhandle) (regValue -> bytes)) = do
      ...
      off <- liftIO $ bvLit sym PtrWidth (BV.mkBV PtrWidth (fromIntegral $ V.length bytes))
      nextptr <- liftIO $ ptrAdd sym PtrWidth ptr off
      setHandle fhandle nextptr
      setFiles fvar files'
...
doOpenFile sym files fident = do
  ...
  z <- bvLit sym PtrWidth (BV.mkBV PtrWidth 0)
...
doCloseFile sym files ptr = do
  ...
  z <- liftIO $ bvLit sym PtrWidth (BV.mkBV PtrWidth 0)
...
doAddFile sym files fident bytes = do
  blkId <- nextBlock (vfsImplBlockSource files)
  size <- bvLit sym PtrWidth (BV.mkBV PtrWidth (fromIntegral $ V.length bytes))
...
doWriteFileBytes sym files ptr bytes = do
  ...
  let store = arrayType (intToNatural (V.length bytes)) (bitvectorType (toBytes (8 :: Int)))
  packed <- packMemValue sym store repr bytes
  (mem', p1, p2) <- G.writeMem sym PtrWidth Nothing ptr store noAlignment packed (vfsMem files)
...
doReadFileBytes sym files ptr size = do
  let repr = VectorRepr (BVRepr (knownNat @8))
  let store = arrayType (bytesToNatural size) (bitvectorType (toBytes (8 :: Int)))
  pv <- G.readMem sym PtrWidth Nothing ptr store noAlignment (vfsMem files)
