...
regEval bak baseEval col ty = go (Just ty)
  where
    ...
    -- evaluated/concretized, if it's known/applicable. It's `Maybe` because
    -- there are cases in which it's unknowable/inapplicable - for instance, in
    -- `goMirRef`, we can't recover the `M.Ty` of the reference's `RefRoot`,
    -- only its `TypeRepr`.
    go ::
      forall tp'.
      Maybe M.Ty ->
    ...
    go (Just (M.TyTuple tys)) MirAggregateRepr (MirAggregate sz m) = do
      -- This relies on our particular approach to tuple layout, in which we put
      -- tuple elements in declaration order in the aggregates we use to
      -- represent them. Rust doesn't necessarily do this; see #1666.
