...
bvPoisonExpr ::
  ...
  NatRepr w ->
  Expr LLVM s (BVType w)
bvPoisonExpr w = App (ExtensionApp (LLVM_BVPoison w))

floatPoisonExpr ::
  FloatInfoRepr fi ->
  Expr LLVM s (FloatType fi)
floatPoisonExpr fi = App (ExtensionApp (LLVM_FloatPoison fi))
...
poisonExpand _archProxy (PtrType _tp) k =
   k proxy# PtrRepr $ BitvectorAsPointerExpr PtrWidth $ bvPoisonExpr PtrWidth
...
poisonExpand _archProxy FloatType k =
  k proxy# (FloatRepr SingleFloatRepr) (floatPoisonExpr SingleFloatRepr)
poisonExpand _archProxy DoubleType k =
  k proxy# (FloatRepr DoubleFloatRepr) (floatPoisonExpr DoubleFloatRepr)
poisonExpand _archProxy X86_FP80Type k =
  k proxy# (FloatRepr X86_80FloatRepr) (floatPoisonExpr X86_80FloatRepr)
