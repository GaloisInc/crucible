...
import           Control.Lens ((^.), view, Prism', re)
...
type HasLLVMAnn sym = (?bbPrism :: BBPrism sym)

newtype BBPrism sym =
  BBPrism (Prism' (SymAnnotation sym W4I.BaseBoolType) (BadBehavior (RegValue' sym)))

injectBB :: HasLLVMAnn sym => BadBehavior (RegValue' sym) -> SymAnnotation sym W4I.BaseBoolType
injectBB bb = bb ^. re psm
 where BBPrism psm = ?bbPrism
...
annotateUB :: (IsSymInterface sym, HasLLVMAnn sym) =>
  ...
  Pred sym ->
  IO (Pred sym)
annotateUB sym ub p = W4I.annotateTerm sym (injectBB (BBUndefinedBehavior ub)) p
...
annotateLE :: (IsSymInterface sym, HasLLVMAnn sym) =>
  ...
  Pred sym ->
  IO (Pred sym)
annotateLE sym le p = W4I.annotateTerm sym (injectBB (BBLoadError le)) p
